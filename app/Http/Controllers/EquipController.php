...
use Config;
use App\Repositories\EquipRepository as Equip;
...
class EquipController extends Controller
{
     ...
     *
     * @param  EquipRepository  $equip
     * @return void
     */
    public function __construct(Equip $equip)
    {
        $this->middleware('auth');
        $this->baseModel = $equip;
        $this->defaultRole = 'student';
        $this->paramid = 'equ';
        $this->paramrole = 'equrole';
        $this->paramclass = 'eclass';
        $this->hdninput = '_equid';
        $this->hdncls = '_cls';
        $this->hdncnm = '_cnm';
        $this->classes = Config::get('constants.EQUIP_CLASSES');
        $this->title = array('header' => 'Equip', 'singular' => 'Equip');
    ...
    {   
        ...
        $tableCols = array('name' => 'Name', 'student_count' => 'Number of Equip Students', 'class_start' => 'Start Date', 'class_end' => 'End Date');
        $urls = array(
            'add' => route('addequip'), 
            'delete' => route('deleteequip'), 
            'edit' => route('editequip'), 
            'view' => route('viewequip')
        );

        return view('members.index', 
            ['title' => $this->title, 'urls' => $urls, 'tableCols' => $tableCols, 'results' => $results, 'dlt_field' => $this->hdninput, 'dlt_act' => 'deleteEquip']
        );
    }
    ...
    {   
        return view('discipleship.add', 
            ['title' => $this->title, 'classes' => $this->classes, 'urls' => array('save' => route('saveequip'), 'cancel' => route('allequip')), 'dlt_act' => 'addEquip' ]
        );   
    }
    ...
    {   
        ...
        $urls = array(
            'add' => route('addequip'),
            'view' => route('viewequip', ["$this->paramid" => $fellowship_id]),  
            'save' => route('saveequip'), 
            'cancel' => route('allequip'),
            'assign' => route('assignequrole', ["$this->paramid" => $fellowship_id]),
            'assignteacher' => route('assignequteacher', ["$this->paramid" => $fellowship_id]),
            'attend' => route('attendequip', ["$this->paramid" => $fellowship_id]),
            'update' => route('saveequip')
        );
        return view('discipleship.edit', 
            ['title' => $this->title, 'urls' => $urls, 'tableCols' => $tableCols, 'fellowship' => $fellowship,'role' => $this->defaultRole, 'class_attendance' => $class_attendance, 'order' => $this->validRoles, 'dlt_field' => $this->hdninput, 'dlt_act' => 'editEquip']
        );
    }
    ...
    public function view(Request $request)
    {
        ...
        $urls = array(
            'cancel' => route('editequip', ["$this->paramid" => $fellowship_id]), 
        );
    ...
    public function assign(Request $request)
    {
        ...
        if(!array_key_exists($fellowship_role, $this->validRoles->toArray())) {
            return redirect()->route('allequip');    
        }
        ...
        $urls = array(
            'save' => route('saveequip'), 
            'edit' => route('assignequrole', ["$this->paramid" => $fellowship_id]),
            'cancel' => route('editequip', ["$this->paramid" => $fellowship_id])
        );
    ...
    public function assignteacher(Request $request)
    {
        ...
        $urls = array(
            'save' => route('saveequip'), 
            'edit' => route('assignequteacher', ["$this->paramid" => $fellowship_id]),
            'cancel' => route('editequip', ["$this->paramid" => $fellowship_id])
        );
    ...
    public function attend(Request $request)
    {
        ...
        $urls = array(
            'save' => route('saveequip'), 
            'edit' => route('attendequip', ["$this->paramid" => $fellowship_id]),
            'cancel' => route('editequip', ["$this->paramid" => $fellowship_id]), 
        );
    ...
    public function destroy(Request $request)
    {
        ...
        $this->validate($request, [
            '_formaction' => 'bail|required|in:deleteEquip,deleteMember',
    ...
    public function save(Request $request)
    {
        ...
        $this->validate($request, [
            '_formaction' => 'bail|required|in:addEquip,editEquip,editRole,markAttendance,editTeacher',
        ]);
        ...
        switch ($action) {
            ...
            case 'editRole':                
                ...
                if($role_limit > 0 && count($member_ids) > $role_limit ) {
                    ...
                    return redirect()->route('assignequrole', ["$this->paramid" => $fellowship_id, "$this->paramrole" => $member_role]);
                }
            ...
            case 'editTeacher':
                ...
                if($role_limit > 0 && count($member_ids) > $role_limit ) {
                    ...
                    $request->session()->flash('message', "Invalid request. There can be no more than $role_limit $member_role for this $name");
                    $request->session()->flash('alert-class', 'alert-danger');
                    return redirect()->route('assignequteacher', ["$this->paramid" => $fellowship_id, "$this->paramclass" => $request->{$this->hdncnm}]);
                }
            ...
            case 'markAttendance':
                ...
                foreach ($class_attendance as $id => $value) {
                    ...
                    $status = $this->baseModel->determineStudentStatus($value, Config::get('constants.FINISH_EQUIP'));
        ...
        return redirect()->route('editequip', [$fellowship_id]);
