...
use Config;
use App\Repositories\EstablishRepository as Establish;
...
class EstablishController extends Controller
{
     ...
     * @return void
     */
    public function __construct(Establish $establish)
    {
        $this->middleware('auth');
        $this->baseModel = $establish;
        $this->defaultRole = 'student';
        $this->paramid = 'est';
        $this->paramrole = 'estrole';
        $this->paramclass = 'eclass';
        $this->hdninput = '_estid';
        $this->hdncls = '_cls';
        $this->hdncnm = '_cnm';
        $this->classes = Config::get('constants.ESTABLISH_CLASSES');
        $this->title = array('header' => 'Establish', 'singular' => 'Establish');
    ...
    {   
        ...
        $tableCols = array('name' => 'Name', 'student_count' => 'Number of Establish Students', 'class_start' => 'Start Date', 'class_end' => 'End Date');
        $urls = array(
            'add' => route('addestablish'), 
            'delete' => route('deleteestablish'), 
            'edit' => route('editestablish'), 
            'view' => route('viewestablish')
        );

        return view('members.index', 
            ['title' => $this->title, 'urls' => $urls, 'tableCols' => $tableCols, 'results' => $results, 'dlt_field' => $this->hdninput, 'dlt_act' => 'deleteEstablish']
        );
    }
    ...
    {   
        return view('discipleship.add', 
            ['title' => $this->title, 'classes' => $this->classes, 'urls' => array('save' => route('saveestablish'), 'cancel' => route('allestablish')), 'dlt_act' => 'addEstablish' ]
        );   
    }
    ...
    {   
        ...
        $urls = array(
            'add' => route('addestablish'),
            'view' => route('viewestablish', ["$this->paramid" => $fellowship_id]),  
            'save' => route('saveestablish'), 
            'cancel' => route('allestablish'),
            'assign' => route('assignestrole', ["$this->paramid" => $fellowship_id]),
            'assignteacher' => route('assignestteacher', ["$this->paramid" => $fellowship_id]),
            'attend' => route('attendestablish', ["$this->paramid" => $fellowship_id]),
            'update' => route('saveestablish')
        );
        return view('discipleship.edit', 
            ['title' => $this->title, 'urls' => $urls, 'tableCols' => $tableCols, 'fellowship' => $fellowship,'role' => $this->defaultRole, 'class_attendance' => $class_attendance, 'order' => $this->validRoles, 'dlt_field' => $this->hdninput, 'dlt_act' => 'editEstablish']
        );
    }
    ...
    public function view(Request $request)
    {
        ...
        $urls = array(
            'cancel' => route('editestablish', ["$this->paramid" => $fellowship_id]), 
        );
    ...
    public function assign(Request $request)
    {
        ...
        if(!array_key_exists($fellowship_role, $this->validRoles->toArray())) {
            return redirect()->route('allestablish');    
        }
        ...
        $urls = array(
            'save' => route('saveestablish'), 
            'edit' => route('assignestrole', ["$this->paramid" => $fellowship_id]),
            'cancel' => route('editestablish', ["$this->paramid" => $fellowship_id])
        );
    ...
    public function assignteacher(Request $request)
    {
        ...
        $urls = array(
            'save' => route('saveestablish'), 
            'edit' => route('assignestteacher', ["$this->paramid" => $fellowship_id]),
            'cancel' => route('editestablish', ["$this->paramid" => $fellowship_id])
        );
    ...
    public function attend(Request $request)
    {
        ...
        $urls = array(
            'save' => route('saveestablish'), 
            'edit' => route('attendestablish', ["$this->paramid" => $fellowship_id]),
            'cancel' => route('editestablish', ["$this->paramid" => $fellowship_id]), 
        );
    ...
    public function destroy(Request $request)
    {
        ...
        $this->validate($request, [
            '_formaction' => 'bail|required|in:deleteEstablish,deleteMember',
    ...
    public function save(Request $request)
    {
        ...
        $this->validate($request, [
            '_formaction' => 'bail|required|in:addEstablish,editEstablish,editRole,markAttendance,editTeacher',
        ]);
        ...
        switch ($action) {
            ...
            case 'addEstablish':
                ...
                $validationrules['name'] = 'bail|required|unique:establish|max:255';
            ...
            case 'editEstablish':
                ...
                $validationrules['name'] = 'bail|required|unique:establish,name,'.$fellowship_id.'|max:255';
                $this->validate($request, $validationrules);
            ...
            case 'editRole':                
                // form validation
                $this->validate($request, [
                    "$this->hdninput" => 'bail|required|integer|exists:establish,id', 
                ...
                if($role_limit > 0 && count($member_ids) > $role_limit ) {
                    ...
                    return redirect()->route('assignestrole', ["$this->paramid" => $fellowship_id, "$this->paramrole" => $member_role]);
                }
            ...
            case 'editTeacher':
                ...
                if($role_limit > 0 && count($member_ids) > $role_limit ) {
                    ...
                    $request->session()->flash('message', "Invalid request. There can be no more than $role_limit $member_role for this $name");
                    $request->session()->flash('alert-class', 'alert-danger');
                    return redirect()->route('assignestteacher', ["$this->paramid" => $fellowship_id, "$this->paramclass" => $request->{$this->hdncnm}]);
                }
            ...
            case 'markAttendance':
                ...
                foreach ($class_attendance as $id => $value) {
                    ...
                    $status = $this->baseModel->determineStudentStatus($value, Config::get('constants.FINISH_ESTABLISH'));
        ...
        return redirect()->route('editestablish', [$fellowship_id]);
